import {
  CalendarClock,
  Cloud,
  Monitor,
  Pencil,
  Puzzle,
  Shield,
} from "lucide-react";
function WHY_SECTION2() {
  const featureSections = [
    {
      icon: <Puzzle className="w-12 h-12 text-cyan-600" />,
      title: "Use Your Own Content",
      description: "Upload and display images, videos, and PDFs with ease.",
    },
    {
      icon: <Pencil className="w-12 h-12 text-cyan-600" />,
      title: "Easy Content Creation",
      description: "Design and publish content with simple templates.",
    },
    {
      icon: <Cloud className="w-12 h-12 text-cyan-600" />,
      title: "Apps & Integrations",
      description: "Connect with social media and popular apps.",
    },
    {
      icon: <Monitor className="w-12 h-12 text-cyan-600" />,
      title: "Multi-Screen Management",
      description: "Control and sync displays across multiple locations.",
    },
    {
      icon: <Shield className="w-12 h-12 text-cyan-600" />,
      title: "Roles & Permissions",
      description: "Manage team access levels and permissions.",
    },
    {
      icon: <CalendarClock className="w-12 h-12 text-cyan-600" />,
      title: "Playlists & Scheduling",
      description: "Automate content with playlists and schedules.",
    },
  ];
  // const featureSections = [
  //   {
  //     icon: (
  //       <svg
  //         xmlns="http://www.w3.org/2000/svg"
  //         width="48"
  //         height="48"
  //         viewBox="0 0 24 24"
  //         fill="none"
  //         stroke="currentColor"
  //         strokeWidth="2"
  //         strokeLinecap="round"
  //         strokeLinejoin="round"
  //         className="lucide lucide-puzzle text-cyan-600"
  //         aria-hidden="true"
  //       >
  //         <path d="M15.39 4.39a1 1 0 0 0 1.68-.474 2.5 2.5 0 1 1 3.014 3.015 1 1 0 0 0-.474 1.68l1.683 1.682a2.414 2.414 0 0 1 0 3.414L19.61 15.39a1 1 0 0 1-1.68-.474 2.5 2.5 0 1 0-3.014 3.015 1 1 0 0 1 .474 1.68l-1.683 1.682a2.414 2.414 0 0 1-3.414 0L8.61 19.61a1 1 0 0 0-1.68.474 2.5 2.5 0 1 1-3.014-3.015 1 1 0 0 0 .474-1.68l-1.683-1.682a2.414 2.414 0 0 1 0-3.414L4.39 8.61a1 1 0 0 1 1.68.474 2.5 2.5 0 1 0 3.014-3.015 1 1 0 0 1-.474-1.68l1.683-1.682a2.414 2.414 0 0 1 3.414 0z"></path>
  //       </svg>
  //     ),
  //     title: "Use Your Own Content",
  //     description:
  //       "Upload and display files like JPG, PNG, GIF, MP4, PDF, and PPT with ease.Seamlessly integrate with multiple file systems for smooth content management.",
  //   },
  //   {
  //     icon: (
  //       <svg
  //         xmlns="http://www.w3.org/2000/svg"
  //         width="48"
  //         height="48"
  //         viewBox="0 0 24 24"
  //         fill="none"
  //         stroke="currentColor"
  //         strokeWidth="2"
  //         strokeLinecap="round"
  //         strokeLinejoin="round"
  //         className="lucide lucide-puzzle text-cyan-600"
  //         aria-hidden="true"
  //       >
  //         <path d="M15.39 4.39a1 1 0 0 0 1.68-.474 2.5 2.5 0 1 1 3.014 3.015 1 1 0 0 0-.474 1.68l1.683 1.682a2.414 2.414 0 0 1 0 3.414L19.61 15.39a1 1 0 0 1-1.68-.474 2.5 2.5 0 1 0-3.014 3.015 1 1 0 0 1 .474 1.68l-1.683 1.682a2.414 2.414 0 0 1-3.414 0L8.61 19.61a1 1 0 0 0-1.68.474 2.5 2.5 0 1 1-3.014-3.015 1 1 0 0 0 .474-1.68l-1.683-1.682a2.414 2.414 0 0 1 0-3.414L4.39 8.61a1 1 0 0 1 1.68.474 2.5 2.5 0 1 0 3.014-3.015 1 1 0 0 1-.474-1.68l1.683-1.682a2.414 2.414 0 0 1 3.414 0z"></path>
  //       </svg>
  //     ),
  //     title: "Easy Content Creation",
  //     description:
  //       "Design and publish content with a simple drag-and-drop editor. Use customizable templates to create professional visuals بسرعة.No design skills needed to get started quickly.",
  //   },
  //   {
  //     icon: (
  //       <svg
  //         xmlns="http://www.w3.org/2000/svg"
  //         width="48"
  //         height="48"
  //         viewBox="0 0 24 24"
  //         fill="none"
  //         stroke="currentColor"
  //         strokeWidth="2"
  //         strokeLinecap="round"
  //         strokeLinejoin="round"
  //         className="lucide lucide-puzzle text-cyan-600"
  //         aria-hidden="true"
  //       >
  //         <path d="M15.39 4.39a1 1 0 0 0 1.68-.474 2.5 2.5 0 1 1 3.014 3.015 1 1 0 0 0-.474 1.68l1.683 1.682a2.414 2.414 0 0 1 0 3.414L19.61 15.39a1 1 0 0 1-1.68-.474 2.5 2.5 0 1 0-3.014 3.015 1 1 0 0 1 .474 1.68l-1.683 1.682a2.414 2.414 0 0 1-3.414 0L8.61 19.61a1 1 0 0 0-1.68.474 2.5 2.5 0 1 1-3.014-3.015 1 1 0 0 0 .474-1.68l-1.683-1.682a2.414 2.414 0 0 1 0-3.414L4.39 8.61a1 1 0 0 1 1.68.474 2.5 2.5 0 1 0 3.014-3.015 1 1 0 0 1-.474-1.68l1.683-1.682a2.414 2.414 0 0 1 3.414 0z"></path>
  //       </svg>
  //     ),
  //     title: "Apps & Integrations",
  //     description:
  //       " Connect with social media, Google Drive, weather, and more.Display dynamic, real-time content from your favorite tools.Bring all your content sources into one platform.",
  //   },
  //   {
  //     icon: (
  //       <svg
  //         xmlns="http://www.w3.org/2000/svg"
  //         width="48"
  //         height="48"
  //         viewBox="0 0 24 24"
  //         fill="none"
  //         stroke="currentColor"
  //         strokeWidth="2"
  //         strokeLinecap="round"
  //         strokeLinejoin="round"
  //         className="lucide lucide-puzzle text-cyan-600"
  //         aria-hidden="true"
  //       >
  //         <path d="M15.39 4.39a1 1 0 0 0 1.68-.474 2.5 2.5 0 1 1 3.014 3.015 1 1 0 0 0-.474 1.68l1.683 1.682a2.414 2.414 0 0 1 0 3.414L19.61 15.39a1 1 0 0 1-1.68-.474 2.5 2.5 0 1 0-3.014 3.015 1 1 0 0 1 .474 1.68l-1.683 1.682a2.414 2.414 0 0 1-3.414 0L8.61 19.61a1 1 0 0 0-1.68.474 2.5 2.5 0 1 1-3.014-3.015 1 1 0 0 0 .474-1.68l-1.683-1.682a2.414 2.414 0 0 1 0-3.414L4.39 8.61a1 1 0 0 1 1.68.474 2.5 2.5 0 1 0 3.014-3.015 1 1 0 0 1-.474-1.68l1.683-1.682a2.414 2.414 0 0 1 3.414 0z"></path>
  //       </svg>
  //     ),
  //     title: "Playlists & Scheduling",
  //     description:
  //       "Create playlists using images, videos, and apps.Schedule content by time, date, or specific events.Automate your screens for consistent and timely updates.",
  //   },
  //   {
  //     icon: (
  //       <svg
  //         xmlns="http://www.w3.org/2000/svg"
  //         width="48"
  //         height="48"
  //         viewBox="0 0 24 24"
  //         fill="none"
  //         stroke="currentColor"
  //         strokeWidth="2"
  //         strokeLinecap="round"
  //         strokeLinejoin="round"
  //         className="lucide lucide-puzzle text-cyan-600"
  //         aria-hidden="true"
  //       >
  //         <path d="M15.39 4.39a1 1 0 0 0 1.68-.474 2.5 2.5 0 1 1 3.014 3.015 1 1 0 0 0-.474 1.68l1.683 1.682a2.414 2.414 0 0 1 0 3.414L19.61 15.39a1 1 0 0 1-1.68-.474 2.5 2.5 0 1 0-3.014 3.015 1 1 0 0 1 .474 1.68l-1.683 1.682a2.414 2.414 0 0 1-3.414 0L8.61 19.61a1 1 0 0 0-1.68.474 2.5 2.5 0 1 1-3.014-3.015 1 1 0 0 0 .474-1.68l-1.683-1.682a2.414 2.414 0 0 1 0-3.414L4.39 8.61a1 1 0 0 1 1.68.474 2.5 2.5 0 1 0 3.014-3.015 1 1 0 0 1-.474-1.68l1.683-1.682a2.414 2.414 0 0 1 3.414 0z"></path>
  //       </svg>
  //     ),
  //     title: "Multi-Screen Management",
  //     description:
  //       "Control one screen or thousands from a single dashboard.Manage content across multiple locations بسهولة.Keep all displays updated in real time.",
  //   },
  //   {
  //     icon: (
  //       <svg
  //         xmlns="http://www.w3.org/2000/svg"
  //         width="48"
  //         height="48"
  //         viewBox="0 0 24 24"
  //         fill="none"
  //         stroke="currentColor"
  //         strokeWidth="2"
  //         strokeLinecap="round"
  //         strokeLinejoin="round"
  //         className="lucide lucide-puzzle text-cyan-600"
  //         aria-hidden="true"
  //       >
  //         <path d="M15.39 4.39a1 1 0 0 0 1.68-.474 2.5 2.5 0 1 1 3.014 3.015 1 1 0 0 0-.474 1.68l1.683 1.682a2.414 2.414 0 0 1 0 3.414L19.61 15.39a1 1 0 0 1-1.68-.474 2.5 2.5 0 1 0-3.014 3.015 1 1 0 0 1 .474 1.68l-1.683 1.682a2.414 2.414 0 0 1-3.414 0L8.61 19.61a1 1 0 0 0-1.68.474 2.5 2.5 0 1 1-3.014-3.015 1 1 0 0 0 .474-1.68l-1.683-1.682a2.414 2.414 0 0 1 0-3.414L4.39 8.61a1 1 0 0 1 1.68.474 2.5 2.5 0 1 0 3.014-3.015 1 1 0 0 1-.474-1.68l1.683-1.682a2.414 2.414 0 0 1 3.414 0z"></path>
  //       </svg>
  //     ),
  //     title: "Roles & Permissions",
  //     description:
  //       " Invite team members and assign custom access levels.Maintain control with approval workflows and permissions.Collaborate efficiently while keeping content secure.",
  //   },
  // ];
  return (
    <>
      <section className="">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
            {featureSections?.map((singleFeature, index) => (
              <div className="flex flex-col items-center p-6 bg-gray-50 rounded-lg shadow-sm text-center">
                <div className="mb-4">
                  {/* Icon */}
                  {singleFeature?.icon}
                </div>
                <h3 className="text-xl md:text-2xl font-bold text-gray-900 mb-4">
                  {singleFeature?.title}
                </h3>
                <p className="text-sm font-semibold text-gray-700">
                  {singleFeature?.description}
                </p>
              </div>
            ))}
          </div>
        </div>
      </section>
    </>
  );
}

export default WHY_SECTION2;
